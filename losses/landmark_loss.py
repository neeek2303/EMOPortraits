...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self,
        ...
        elif network == 'vgg_emotions':
            self.model = _vgg_emotions(layers)
        elif network == 'resnet_emotions':
            self.model = _vgg_emotions_resnet(layers)
        else:
    ...
    def forward(self,
                ...
                mask=None) -> Union[torch.Tensor, list]:
        ...
        if self.resize:
            ...
            if self.gray:
                ...
                one_channel_input = inputs[:, 0, :, :].unsqueeze(1) * 0.299 + inputs[:, 1, :, :].unsqueeze(
                    1) * 0.587 + inputs[:, 2, :, :].unsqueeze(1) * 0.114
                one_channel_target = target[:, 0, :, :].unsqueeze(1) * 0.299 + target[:, 1, :, :].unsqueeze(
                    1) * 0.587 + target[:, 2, :, :].unsqueeze(1) * 0.114
...
def _vgg_emotions(layers):
    r"""Get vgg face layers"""
    network = VGG('VGG19')
    state_dict = torch.load(
        '/Vol0/user/n.drobyshev/avatars/march_refactor/latent-texture-avatar/losses/loss_model_weights/PrivateTest_model.t7',
...
def _vgg_emotions_resnet(layers):
    r"""Get vgg face layers"""
    network = ResNet18_ARM___RAF()
    state_dict = torch.load(
        '/Vol0/user/n.drobyshev/avatars/march_refactor/latent-texture-avatar/losses/loss_model_weights/epoch59_acc0.9205.pth',
