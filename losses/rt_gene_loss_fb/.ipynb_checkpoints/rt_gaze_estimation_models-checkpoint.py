...
class GazeEstimationAbstractModel(nn.Module):
    ...
    def _create_fc_layers(in_features: int, out_features: int):
        x_l = nn.Sequential(
        ...
        fc = nn.Sequential(
            nn.Linear(514, 256), nn.ReLU(inplace=True), nn.Linear(256, out_features)
        )
...
class GazeEstimationModelResnet18(GazeEstimationAbstractModel):
    ...
    def __init__(self, num_out=2):
        ...
        (
            ...
            # pyre-fixme[4]: Attribute must be annotated.
            self.fc,
        ) = GazeEstimationAbstractModel._create_fc_layers(
...
class GazeEstimationModelPreactResnet(GazeEstimationAbstractModel):
        ...
        def _make_stage(in_channels, out_channels, n_blocks, block, stride):
            ...
            for index in range(n_blocks):
                ...
                if index == 0:
                    ...
                    stage.add_module(
                        block_name, block(out_channels, out_channels, stride=1)
                    )
            return stage
    ...
    def __init__(self, num_out=2):
        ...
        (
            ...
            in_features=64, out_features=num_out
        )
        GazeEstimationAbstractModel._init_weights(self.modules())
...
class GazeEstimationModelVGG(GazeEstimationAbstractModel):
    ...
    def __init__(self, num_out=2):
        ...
        # pyre-fixme[16]: Module `models` has no attribute `vgg16`.
        ...
        (
            ...
            in_features=_left_model.classifier[0].in_features, out_features=num_out
        )
        GazeEstimationAbstractModel._init_weights(self.modules())
