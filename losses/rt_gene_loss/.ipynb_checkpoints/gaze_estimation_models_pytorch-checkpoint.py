...
class EyesEncoderVGG(nn.Module):
    def __init__(self, eyes_out=128):
        super(EyesEncoderVGG, self).__init__()
        self.out_features = eyes_out
        self.sub_model = torch.nn.Sequential(*list(models.vgg16_bn().features.children())[1:])
        self.new_first = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        self.classifier = torch.nn.Sequential(*list(models.vgg16_bn().classifier.children())[:-4])
...
class EyesEncoderResnet(nn.Module):
    def __init__(self, eyes_out=256):
...
class GazeEstimationModel(nn.Module):
    def __init__(self, eyes_out=256, kpts_out=64, use_bias=False, model_type='vgg16', device='cuda'):
        ...
        self.keypoints_encoder = KeypointsEncoder(num_inputs=68, num_harmonics=8, num_channels=128, num_layers=3,
                                                  output_channels=kpts_out, output_size=1)
    ...
    def get_eye_embeddings(self, left_eye, right_eye, layer_indices):
        ...
        if self.model_type == 'vgg16':
            ...
            def prepare_features(model):
                result = [model.conv1, model.bn1, model.relu, model.maxpool, model.layer1, model.layer2, model.layer3, model.layer4, model.avgpool]
                return nn.Sequential(*result)
            self.prepared_eyes_encoder = prepare_features(self.eyes_encoder.resnet18) 
             
  
        left_embeddings = extract_embeddings(self.prepared_eyes_encoder, left_eye.to(self.device))
        right_embeddings = extract_embeddings(self.prepared_eyes_encoder, right_eye.to(self.device))

        return left_embeddings, right_embeddings

    def forward(self, left_eye, right_eye, kp2d, mode='full'):
