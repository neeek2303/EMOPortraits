...
from .utils import torch_warpaffine, FaceBox
...
class TrackedSubject(object):
    ...
    def compute_eye_images_from_landmarks(self, eye_image_size):
        ...
        left_eye_color_resized = F.interpolate(
            left_eye_color.unsqueeze(0), eye_image_size[::-1], mode='bicubic', align_corners=False).squeeze(0)
        right_eye_color_resized = F.interpolate(
            right_eye_color.unsqueeze(0), eye_image_size[::-1], mode='bicubic', align_corners=False).squeeze(0)
...
#         print(desired_face_width,desired_face_width)
        ...
        scale = desired_dist / max(dist, 1)  # max(..., 1) is a workaround for division by zero
