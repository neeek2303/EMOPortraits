...
class Logger(object):
    def __init__(self, args, experiment_dir, rank, model, project_name = "main_gig", entity="animator"):
    # def __init__(self, args, experiment_dir, rank, model, project_name = "main_model_MP2"):
        ...
        if self.rank == 0:
            ...
            co = glob(f'{args.project_dir}/*.py')
            co += glob(f'{args.project_dir}/datasets/*.py')
            co += glob(f'{args.project_dir}/*.txt')
            co += glob(f'{args.project_dir}/models/*.py')
            co += glob(f'{args.project_dir}/models/stage_1/volumetric_avatar/*.py')
            co += glob(f'{args.project_dir}/models/stage_2/volumetric_avatar/*.py')
            co += glob(f'{args.project_dir}/networks/volumetric_avatar/*.py')

            if self.log_data:
                # wandb.init(project=project_name, entity="neeek2303", save_code=True)
                wandb.init(project=project_name, entity=entity, save_code=True)
                for path in co:
                    wandb.save(path)
    ...
    def log(self, phase, losses_dict = None, visuals = None, epoch_end = False, explaining_var=None):
        ...
        if phase == 'train':
            ...
            if visuals is not None:
                self.output_visuals(phase, visuals, explaining_var)
    ...
    def output_logs(self, phase):
        ...
        if self.rank == 0:
            ...
            if phase=='test':
                wandb.log({'epoch':self.epoch}, step=self.train_iter)

            tqdm.write(f'Iter {self.train_iter:06d} ' + ', '.join(f'{name}: {losses[-1]:.3f}' for name, losses in self.losses[phase].items()))

    def output_visuals(self, phase, visuals, explaining_var = None):
        ...
        if self.rank == 0 or device == 'cpu':
            ...
            if phase=='test' and explaining_var is not None:
                print('show explaining var')
                plt.rcParams["figure.figsize"] = [7.50, 3.50]
                plt.rcParams["figure.autolayout"] = True

                plt.figure()
                # plt.plot([i for i in range(len(explaining_var))], explaining_var)

                plt.scatter(x=[i+1 for i in range(len(explaining_var))],
                            y=explaining_var,
                ...
                wandb.log({f'"Explaining variance"': [wandb.Image(im)]}, step=self.train_iter)
                ##############################################################################################

                print('show explaining var log')
                plt.rcParams["figure.figsize"] = [7.50, 3.50]
                plt.rcParams["figure.autolayout"] = True
                ...
                im = Image.open(img_buf)
                wandb.log({f'"Explaining variance log"': [wandb.Image(im)]}, step=self.train_iter)
