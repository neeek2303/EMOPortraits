...
from utils.weight_std import weight_std
...
class BasicBlock(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            if norm_layer_type != 'none':
                layers_ += [norm_layer(in_channels if i == 0 else out_channels, affine=True)]
...
class BottleneckBlock(nn.Module):
    def __init__(
        ...
        if use_weight_std:
            layers_[-1] = weight_std(layers_[-1])

        if norm_layer_type != 'none':
            layers_ += [norm_layer(out_channels, affine=True)]
...
norm_layers = {
    ...
    'ada_in_3d': norms.AdaptiveInstanceNorm,
    'ada_gn': lambda num_features, affine=True: norms.AdaptiveGroupNorm(num_groups=32, num_features=num_features, affine=affine)}
